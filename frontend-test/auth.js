...
async function handleCredentialResponse(response) {
    ...
    try {
        ...
        if (backendResponse.ok) {
            ...
            showSuccess(`Welcome ${data.user.name}! Google authentication successful.`);
            onLoginSuccess(data);
        } else {
...
function onLoginSuccess(data) {
    ...
    document.getElementById('user-role').textContent = getRoleName(data.user.role);
    document.getElementById('user-district').textContent = data.user.district || 'Not specified';
    document.getElementById('user-google').textContent = data.user.is_google ? 'Yes' : 'No';
    ...
    document.getElementById('signup-form').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showSuccess(`Account created successfully for ${data.user.name}! Please login now.`);
...
function clearTestResults() {
    const sections = ['auth-status', 'issues-status', 'user-status', 'authority-status'];
    sections.forEach(id => {
    ...
    ['auth-response', 'issues-response', 'user-response', 'authority-response'].forEach(id => {
...
async function testGetIssues() {
    ...
    if (response && response.ok) {
        const data = await response.json();
        document.getElementById('issues-response').textContent = JSON.stringify(data, null, 2);
...
async function testCreateIssue() {
    ...
    if (!authority_id) {
        document.getElementById('issues-response').textContent = 'No existing issues found to get authority_id. Cannot test issue creation.';
        updateStatus('issues-status', false);
        return;
    }
